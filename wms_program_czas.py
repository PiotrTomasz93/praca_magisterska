...
def ZapiszObraz(obiekt_powiatu, wms, warstwa, size = (1200,800), format = 'image/png', transparent = True, bgcolor = '#FFFFFF'):
...
def Raport(lista_bledow = [], czas = 0):
    ...
    if blad_2 == 1:
        ...
        arkusz.cell(row = wiersz, column = kolumna_powiatu + 1).value = str(bledy)
        print "Liczba bledow: " + str(bledy)
        print 'Czas: ' + czas + ' sekund' + '\n'

def ZapiszObraz100(obiekt_powiatu, wms, warstwa, liczba_obrazow = 100, size = (300,300), format = 'image/png', transparent = True, bgcolor = '#FFFFFF'):
    ...
    for linia in plik_z_punktami:
        ...
        if k > liczba_obrazow:
            break
        punkt = linia.split('\t')
        X1 = float(punkt[3]) - 150
        Y1 = float(punkt[2]) - 150
        X2 = float(punkt[3]) + 150
        Y2 = float(punkt[2]) + 150
        bb92 = (Y1, X1, Y2, X2)
...
arkusz = excel.get_sheet_by_name('czas')
...
for linia in plik_z_powiatami:
    ...
    if obiekt_powiatu.stan == 'Dziala':
        ...
        X1 = float(obiekt_powiatu.X_p_losowego) - 150
        Y1 = float(obiekt_powiatu.Y_p_losowego) - 150
        X2 = float(obiekt_powiatu.X_p_losowego) + 150
        Y2 = float(obiekt_powiatu.Y_p_losowego) + 150
        bb92 = (Y1, X1, Y2, X2)
        ...
        ZapiszObraz100(obiekt_powiatu,wms,obiekt_powiatu.nazwa_warstwy)
